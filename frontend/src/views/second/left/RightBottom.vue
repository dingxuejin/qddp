<template>
  <transition name='opacity1'>
    <div class="echart-container">
      <!-- 并网数量俩柱状折线图切换 -->
      <v-chart :options='newOptions' ref="bw"
               :auto-resize='true' />
...
export default {
      ...
      if (this.status === 'right1') {
        ...
        if (this.gfqxfb && this.gfqxfb.length > 0) {
          ...
          this.gfqxfb.forEach(item => {
            ...
            data1.push(item.BWSL_17)
            data2.push(item.BWSL_18)
            data3.push(item.BWSL_19)
          })
        ...
        let name2 = yearArr[1]
        let name3 = yearArr[2]
        let data1 = data[yearArr[0]].map(val => val.BWSL)
        let data2 = data[yearArr[1]].map(val => val.BWSL)
        let data3 = data[yearArr[2]].map(val => val.BWSL)
        options.xAxis[0].data = xData
  ...
  destroyed () {
    if (this.$refs.bw && this.$refs.bw.dispatchAction) {
      this.$refs.bw.dispatchAction({
        type: 'hideTip'
      })
    }
